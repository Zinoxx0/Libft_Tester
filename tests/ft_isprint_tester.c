...
int	main(void)
{
	...
	printf("\n\033[1;36m      🧪 Testing ft_isprint Function     \033[0m\n\n");
	passed = 0;
	passed += test("space boundary", ft_isprint(' '));
	passed += test("tilde boundary", ft_isprint('~'));
	passed += test("newline rejected", !ft_isprint('\n'));
	passed += test("DEL rejected", !ft_isprint(127));
